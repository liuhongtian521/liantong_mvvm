...
public class WpsUtil {
    ...
    public void openDocument(File file) {
        try {
            ...
            if (file != null) {
                openDocWithSimple(file, mActivity);
            } else {
                openDocFile();
            }
        } catch (Exception e) {
    ...
    public static void openDocWithSimple(File file, Context context) {
        try {
            ...
            bundle.putBoolean(Define.EDIT_MODE, true);
...
// 避免从桌面启动程序后，会重新实例化入口类的activity
        Intent intent = null;
        try {
            intent = mActivity.getPackageManager().getLaunchIntentForPackage("cn.wps.moffice_eng");
            if (intent == null) {
                Toast.makeText(mActivity, "未安装WPS", Toast.LENGTH_SHORT).show();
            }
            Bundle bundle = new Bundle();
            if (canWrite) {
                bundle.putString(Define.OPEN_MODE, Define.NORMAL);
